...
const inicializarTablas = async () => {
    try {
        ...
        `);
        console.log("⚡ Estructuras de tablas en Neon verificadas.");
    } catch (err) {
        console.error("❌ Falló inicialización de tablas:", err);
    }
};
inicializarTablas();

// =========================================================================
// --- RUTAS DE AUTENTICACIÓN Y CONTROL DE ACCESO (PROTEGIDO) ---
// =========================================================================
app.post('/api/auth/login', async (req, res) => {
    // Protección contra variables undefined en la desestructuración
    ...
    // Admin estático por bocetos de la profesora
...
// =========================================================================
// --- COPIAS DE SEGURIDAD INTERNAS: COMPRESIÓN COMPLETA EN JSON ---
// =========================================================================
...
app.post('/api/auth/login', async (req, res) => {
    ...
    // Admin estático por bocetos de la profesora
    if (username === 'profesora' && password === 'admin123') {
        req.session.usuario = { id: 0, username: 'profesora', rol: 'profesora' };
        return res.json({ success: true, rol: 'profesora' });
    }

    try {
        const resultado = await pool.query('SELECT * FROM usuarios WHERE LOWER(username) = $1', [username.toLowerCase().trim()]);
        if (resultado.rows.length === 0) {
            return res.status(401).json({ success: false, message: "El usuario no existe." });
        }
...
// =========================================================================
// --- RUTAS DE ADMINISTRACIÓN: CONTROL DE ESTUDIANTES (REPARADO Y BLINDADO) ---
// =========================================================================
...
// ENPOINT REPARADO: Creación estricta de alumnos para evitar errores 500 en Neon
app.post('/api/usuarios', async (req, res) => {
    const { username, password, rol, curso_id } = req.body;
    ...
    try {
        // Sanitizar el ID del curso: si viene corrupto, indefinido o NaN, lo guardamos como null
        const limpioCursoId = (curso_id && !isNaN(parseInt(curso_id))) ? parseInt(curso_id) : null;
        ...
        console.error("❌ Error interno al insertar usuario en Neon:", err.message);
        
        if (err.code === '23505') { // Código de PostgreSQL para llave duplicada (username único)
            return res.status(400).json({ success: false, error: "El nombre de usuario ya está registrado." });
        }
        res.status(500).json({ success: false, error: "Error de persistencia: " + err.message });
    }
